...
const ColorPieChart = () => {
  ...
  const allColorNames = items.flatMap(
    (item) =>
      item.colors
        .map((color) => color?.name) // optional chaining: color might be undefined or null
        .filter((name): name is string => typeof name === "string") // only keep strings
  );

  const colorCountMap: { [colorName: string]: number } = {};
  allColorNames.forEach((colorName) => {
    if (!colorName) return; // safety check in case of falsy values
  ...
  return (
    <div style={{ width: "100%", height: 300, backgroundColor: "#222" }}>
