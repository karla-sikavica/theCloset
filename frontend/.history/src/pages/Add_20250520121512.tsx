...
const Add = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!clothingItem.name.trim()) return alert("Name is required");
    if (!clothingItem.brand.trim()) return alert("Brand is required");
    if (!clothingItem.material.trim()) return alert("Material is required");
    if (!clothingItem.price || isNaN(Number(clothingItem.price)))
      return alert("Enter a valid price");
    if (!clothingItem.category) return alert("Please select a category");
    if (!clothingItem.subcategory) return alert("Please select a subcategory");
    if (!clothingItem.size) return alert("Please select a size");
